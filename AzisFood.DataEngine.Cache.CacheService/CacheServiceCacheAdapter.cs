...
public class CacheServiceCacheAdapter : ICacheAdapter
{
    ...
    {
        return _cacheService.HashGetAllAsync<TEntity>(CommandFlags.None);
    }

    /// <inheritdoc />
    public Task<TEntity> GetSingleFromHashAsync<TEntity>(Guid key) where TEntity : class, IRepoEntity, new()
    {
        return _cacheService.HashGetAsync<TEntity>(key.ToString(), CommandFlags.None);
    }

    /// <inheritdoc />
    public Task<IEnumerable<TEntity>> GetFromSingleKeyAsync<TEntity>(string entityName)
    ...
    /// <inheritdoc />
    public async Task<TEntity?> GetFromSingleKeyAsync<TEntity>(string entityName, Guid key)
        where TEntity : class, IRepoEntity, new()
    {
    ...
    /// <inheritdoc />
    public Task DropHashAsync<TEntity>()
    {
        return _cacheService.HashDropAsync<TEntity>();
    }

    /// <inheritdoc />
    public Task StoreItemsAsHashAsync<TEntity>(IEnumerable<TEntity> items)
    {
        return _cacheService.HashSetAsync(items, CommandFlags.None);
    }
    ...
    /// <inheritdoc />
    public Task<bool> StoreItemsAsSingleKeyAsync<TEntity>(string entityName, IEnumerable<TEntity> items,
        TimeSpan expiry)
    {
